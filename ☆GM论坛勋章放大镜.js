...
(function () {
    ...
    function 统计升级消耗(放大镜内容映射表) {
        // 初始化每个对象的消耗统计对象
        var 对象消耗统计 = {}

        // 遍历映射表
        for (var 对象名称 in 放大镜内容映射表) {
            var 对象描述 = 放大镜内容映射表[对象名称]
            var 升级条件 = 对象描述.match(/消耗([-\d]+)\s*(金币|血液|旅程|咒术|知识|灵魂|堕落)/g)
        ...
        var 结果映射表 = {}
        for (var 对象名称 in 放大镜内容映射表) {
            var 消耗描述 = Object.entries(对象消耗统计[对象名称])
...
// 调用函数并输出结果
var 放大镜内容映射表 = 统计升级消耗(放大镜内容映射表raw);
创建控制面板();
初始化放大镜();
变化检测();
})();
